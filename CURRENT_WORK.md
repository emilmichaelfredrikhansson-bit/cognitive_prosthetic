...
- startup-cleanup hardening was committed/pushed as `554ad8a167ed6b45f2d6d354b725b9a1ca6a5193`; a clean Local Companion restart then returned Bob API running and bridge `ready=true`, proving the earlier missing-input startup was transient rather than a durable selector break;
- the deadline extension request had in fact succeeded before that failed restart: the same `campaign-fc8010321446` is RUNNING with the same `run-02b654ab1469`, deadline `2026-09-26T21:47:12Z`, and no replacement canary/run was created;
- resumed slice rounds 33-44 proved benign duplicate-READ recovery LIVE: the run progressed twelve further rounds without replay/block, preserved prior effects, then Bob independently produced a fourth verified effect `arch02-e4`, replacing `bob/driver.py` and advancing isolated head to `93e5ba746c27a1a6eec5d45173c38620fa61ae86`; that refactor reduced `driver.py` from ~966 lines to ~90 while consuming the three Bob-created runtime modules. Slice ended correctly as `ROUND_YIELD` at `campaign-checkpoint-acb20f831988`;
- the next slice progressed through round 52 and then blocked before submission with `CHATGPT_SUBMISSION_FAILED:SEND_CONTROL_DISABLED`. No request/effect replay occurred and effect_count remained 4. Contemporaneously the operator observed ChatGPT's status that its systems were still processing the request a little longer, strongly indicating a transient post-response processing/quiescence window rather than a Bob protocol failure;
- working copy now treats visible-disabled send control / ChatGPT processing status as bounded pre-submission readiness: no Enter/request occurs until the control becomes ready; known RATE_LIMITED/USAGE_LIMIT remains immediate fail-closed; the Swedish/English processing banner is detected separately and never misclassified as a post-submission error. Focused Local Companion **39/39 PASS**, `py_compile`, module-cap regression and `git diff --check` PASS; full suite **227/227 PASS**.

Still required before Priority A is complete:
- checkpoint/push/restart the bounded send-readiness hardening; no canonical/main promotion;
- continue the **same** V2 campaign/run from clean isolated head `93e5ba746c27a1a6eec5d45173c38620fa61ae86` and its four durable verified effects, across as many bounded slices as needed, until deterministic verification succeeds or another genuine fail-closed blocker/deadline occurs;
...
1. **`BOB_SHORT_COGNITION_SLICES_V1` deterministic tranche is GREEN at `67be7c5`.** Campaign execution now hard-yields after one fresh cognition round, persists verified READ/EFFECT/protocol/continuation state plus a round-yield checkpoint, and resumes through a new cognition identity on the next cycle/restart. The operating module target is 14,900 tokens while 15,000 remains the actual hard cap. Targeted tests passed 13/13 and the full suite passed 227/227. Next step is live qualification by resuming the existing self-hosting V2 campaign through repeated short slices; do not lengthen a single ChatGPT conversation to make progress.
2. Reconstruct live local/GitHub/runtime truth before every further step. Latest verified implementation checkpoint on working-branch local/origin is `67be7c52a7e822d3aefed69c138815b2b52b3e98` (`Implement short cognition slices`); canonical `feat/bob-core-v1` remains `098e6ac9f2adb47e7174c4db8ad2d0b1639d1279`.
3. Preserve and continue the **same** self-hosting V2 identity after the short-slice tranche: `campaign-fc8010321446` / `campaign-work-5183bb278080` / `run-02b654ab1469`, isolated branch `bob/run/run-02b654ab1469`, with its **four** live-verified effects intact (`runtime_effects.py`, `runtime_module.py`, `runtime_bridge.py`, then the `driver.py` replacement; isolated HEAD `93e5ba746c27a1a6eec5d45173c38620fa61ae86` at the latest clean check). Do not create a replacement canary or restart from the pre-repair base.
4. Reconcile Local Companion/runtime state, complete a clean restart if still required, and verify Bob API + cognition bridge health before sending further cognition. Preserve the startup-cleanup hardening already pushed at `554ad8a...`.
5. Extend/resume the same V2 campaign only through the explicit bounded campaign-deadline continuation path, then let it progress through as many **short fresh cognition slices** as needed. Benign duplicate READs may receive deterministic protocol correction; duplicate/mismatched effects remain fail-closed.
6. When Bob reaches `BOB.DONE`, run deterministic verification against the exact isolated run/worktree: full suite, module graph caps, clean tree, exact branch/head, public API and authority/effect boundaries, with `promotion_authority=NONE` and no merge/push/promotion.
7. If self-hosting is genuinely green, update this handoff, checkpoint/commit/push the stable working branch, close Priority A and proceed directly toward `FULL_REPLACEMENT_GATE`. Promotion/merge to `feat/bob-core-v1` or `main` still requires explicit operator approval.
...
## LATEST_CHECKPOINT_2026-09-26_SHORT_SLICE_LIVE_QUALIFICATION

- Live qualification resumed the exact existing V2 campaign/item/run after reconciling and restarting Local Companion onto the short-slice implementation; no replacement canary/base restart/promotion occurred.
- Fresh cognition rounds **53..67** were exercised through the same durable run. Rounds 53..66 repeatedly produced clean `ROUND_YIELD` checkpoints and preserved the isolated branch/effect history; the executor remained one fresh ChatGPT conversation per slice.
- Live truth corrected the old handoff count from three to **four** verified isolated effects. The run branch contains `runtime_effects.py`, `runtime_module.py`, `runtime_bridge.py`, and the reduced `driver.py`; no later effect was produced during this qualification window.
- Real bridge pathologies were reproduced without repository mutation: the configured 12 s submission-materialization budget could stretch to several minutes because individual synchronous DOM probes could outlive the Python loop deadline; round 67 failed closed as `BLOCKED_COGNITION: Request timed out`; a concurrent Local Companion restart caused round 68 to fail closed with `ConnectionResetError`; and round 69 exposed a 30 s `Locator.click` pointer-interception failure from the visible `Continue Bob campaign work` overlay. None changed effects, branch HEAD or authority.
- Hardening now checks backend acceptance/error before DOM work, uses a Playwright `wait_for_function` probe with an explicit timeout bounded by the remaining submission budget, and focuses the composer instead of pointer-clicking it while bounding composer focus/press/fill operations to 5 s. New regressions cover backend-first acceptance, bounded production DOM probing and the focus-based composer path.
- Qualification after the hardening: focused submission tests **6/6 PASS**, full Local Companion tests **41/41 PASS**, full repository suite **229/229 PASS**, and `git diff --check` PASS.
- Immediate next action after checkpoint/restart: resume **the same** V2 item from its durable blocked boundary with one short fresh cognition slice, confirm wall-clock submission behavior, and continue until `BOB.DONE` + deterministic isolated-run verification or a genuine fail-closed blocker.
...
Reconstruct actual local/GitHub/runtime state first; this document is never truth when live reality differs. Latest verified implementation checkpoint on working-branch local/origin is `67be7c52a7e822d3aefed69c138815b2b52b3e98` (`Implement short cognition slices`); canonical `feat/bob-core-v1` remains `098e6ac9f2adb47e7174c4db8ad2d0b1639d1279`. **`BOB_SHORT_COGNITION_SLICES_V1` is now deterministically green; immediate next work is live qualification.** Reconcile/restart Local Companion as needed and continue the same self-hosting V2 campaign `campaign-fc8010321446` / item `campaign-work-5183bb278080` / run `run-02b654ab1469` from its existing isolated branch and four verified effects; do not create a new canary or restart from base. Continue across as many bounded short slices as needed until `BOB.DONE` plus deterministic verification or a genuine fail-closed blocker. No merge/promotion to `feat/bob-core-v1` or `main` without explicit operator approval. Resale Engine remains paused until `FULL_REPLACEMENT_GATE` is live-green.
