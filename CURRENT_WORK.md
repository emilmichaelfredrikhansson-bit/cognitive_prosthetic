...
1. **NEXT IMPLEMENTATION TRANCHE — `BOB_SHORT_COGNITION_SLICES_V1`.** Make the newly canonical long-work rule executable before scaling the self-hosting campaign further: a persistent campaign/work item must advance through many intentionally short fresh ChatGPT conversations, each solving one bounded semantic step and persisting its result/request/effect/blocker/continuation before the conversation is discarded. Campaign duration may be hours/days; individual cognition chats must remain short-lived and must never be the continuity mechanism. Define and deterministically test slice exit/budget policy, durable continuation/checkpoint handoff, bounded context recompilation and resume semantics without weakening effect authority or verification.
2. Reconstruct live local/GitHub/runtime truth before every further step. Latest verified working-branch local/origin HEAD at this handoff is `554ad8a167ed6b45f2d6d354b725b9a1ca6a5193` (`Preserve primary runtime startup failures`); canonical `feat/bob-core-v1` remains `098e6ac9f2adb47e7174c4db8ad2d0b1639d1279`.
3. Preserve and continue the **same** self-hosting V2 identity after the short-slice tranche: `campaign-fc8010321446` / `campaign-work-5183bb278080` / `run-02b654ab1469`, isolated branch `bob/run/run-02b654ab1469`, with its three already verified effects intact. Do not create a replacement canary or restart from the pre-repair base.
...
Reconstruct actual local/GitHub/runtime state first; this document is never truth when live reality differs. Latest verified clean working-branch local/origin HEAD at this handoff is `554ad8a167ed6b45f2d6d354b725b9a1ca6a5193` (`Preserve primary runtime startup failures`); canonical `feat/bob-core-v1` remains `098e6ac9f2adb47e7174c4db8ad2d0b1639d1279`. **Immediate next implementation work is `BOB_SHORT_COGNITION_SLICES_V1`: make long-running campaigns advance through many deliberately short, fresh ChatGPT cognition conversations with durable Bob-owned continuation/checkpoint state between every slice. Long campaign duration must scale the number of cognition slices, not the lifespan/context of one chat.** After that tranche is deterministically green, reconcile/restart Local Companion as needed and continue the same self-hosting V2 campaign `campaign-fc8010321446` / item `campaign-work-5183bb278080` / run `run-02b654ab1469` from its existing isolated branch and three verified effects; do not create a new canary or restart from base. Continue across as many bounded short slices as needed until `BOB.DONE` plus deterministic verification or a genuine fail-closed blocker. No merge/promotion to `feat/bob-core-v1` or `main` without explicit operator approval. Resale Engine remains paused until `FULL_REPLACEMENT_GATE` is live-green.
