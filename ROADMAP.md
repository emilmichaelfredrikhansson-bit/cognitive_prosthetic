...
- [x] repo-native `BOB_MODULE_GRAPH_V1` implemented with stable workspace/repository binding, path ownership, dependencies and explicit partial coverage;
- [ ] broaden module/project graph contract to full input/output/failure/adapters/state/verification/evidence semantics across real projects;
- [x] deterministic versioned module footprint measurement wired to graph-owned source/test/contract paths;
- [x] >15k detection routes module cognition into `ARCHITECTURE_REPAIR` mode and blocks DONE while oversized;
- [ ] prove an end-to-end >15k repair that rewrites the graph/modules and returns all affected modules to compliance under existing approval authority;
- [ ] product-vision/mission bootstrap persists operator intent and success criteria;
- [x] Context Compiler V1 produces bounded module packets with graph state, source pointers, invariants and durable continuation;
- [ ] make stateless module selection/context compilation the default production path for ordinary Bob chat;
- [x] module-path READ/effect continuation recompiles the full bounded problem into a fresh cognition request;
- [x] Context Compiler V1 includes exact repository/ref/path pointers so fresh cognition can re-ground directly in connected GitHub reality;
- [x] module cognition prompt requires relevant contract/module/product impact checking before completion;
